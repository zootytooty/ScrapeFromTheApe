...
def brunswick_green_event_parser(in_event, base_url : str):
    """Main process to enrich & return gig data
    
    Args:
        in_event (scrapy.http.response.html.HtmlResponse): Brunswick Green individual show html response portion
...
def parse_event_cost(descr: str):
    """Find and make nice the gig cost from a single event from Brunswick Green website
    
    Args:
        descrip_html (str): The html description string for the single event from Brunswick Green website
...
def get_start_time(in_event):
    ...
    if temp_time is not None:
        ...
        #Brunswick Green format can be 'Time: 0:00 pm - 0:00 pm'
